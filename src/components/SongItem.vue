...
<template>
  <div class="song-item card" :data-id="song.id" :data-encore-marker="isEncoreMarker ? 'true' : undefined"
    :class="{ 'is-encore': isEncore, 'is-marker': isEncoreMarker }">
    <div v-if="isMarker" class="marker-mode">
      <span class="marker-pill">&lt;encore&gt;</span>
      <button v-if="!markerIsLast" class="icon-btn delete no-print marker-reset-btn" type="button"
        title="Reset encore marker to end" aria-label="Reset encore marker to end"
