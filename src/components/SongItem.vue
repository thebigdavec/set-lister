...
<template>
	<div ref="songItemRef" class="song-item card" :data-id="song.id"
		...
		<template v-else>
			<SongItemDisplay v-if="!isEditing" ref="songItemFocusRef" :song="song" :song-number="songNumber"
			...
			<SongItemEdit v-else ref="songItemEditRef" :title="editTitle" :song-key="editKey || ''" @save="
				({ title, key }) => {
					editTitle = title;
					editKey = key;
					save();
