...
const anchorName = ref(`--tooltip-anchor-${Math.random().toString(36).slice(2, 11)}`);
...
const tooltipText = computed(() => {
    ...
    const trigger = triggerRef.value?.querySelector("[title]") as HTMLElement | null;
...
function restoreTitleAttributes(): void {
    if (!triggerRef.value) return;
    const elementsWithDataTitle = triggerRef.value.querySelectorAll("[data-original-title]");
