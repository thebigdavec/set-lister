...
describe('useFileOperations', () => {
  ...
  describe('saveToDisk', () => {
    ...
    describe('with File System Access API', () => {
      ...
      it('should suggest copy filename with -01 suffix when Save As on existing file', async () => {
        ...
        windowAny.showSaveFilePicker = vi.fn().mockResolvedValue(existingHandle)
  ...
  describe('loadFromDisk', () => {
    ...
    describe('with File System Access API', () => {
      ...
      it('should migrate legacy data without schemaVersion', async () => {
        const legacyData = {
          metadata: {
          ...
          sets: [
            {
              id: 'set-1',
              songs: [{ id: 'song-1', title: 'Legacy Song', key: 'C' }],
              metrics: {
                longestEntryId: null,
                longestEntryText: '',
        ...
        const mockFile = {
          text: vi.fn().mockResolvedValue(JSON.stringify(legacyData))
        }
        const mockHandle = {
          getFile: vi.fn().mockResolvedValue(mockFile),
          name: 'legacy-show.json'
        }
      ...
      it('should handle data that already has schemaVersion', async () => {
        const currentData = {
          ...
          metadata: {
            ...
            venue: 'Current Venue',
            date: '2024-01-01',
            actName: 'Current Band'
          },
          sets: [
            {
              id: 'set-1',
              songs: [{ id: 'song-1', title: 'Current Song' }],
              metrics: {
                longestEntryId: null,
                longestEntryText: '',
                longestEntryWidth16px: 0,
        ...
        const mockFile = {
          text: vi.fn().mockResolvedValue(JSON.stringify(currentData))
        }
        const mockHandle = {
          getFile: vi.fn().mockResolvedValue(mockFile),
          name: 'current-show.json'
        }
  ...
  describe('handleFileInputChange', () => {
    ...
    it('should migrate legacy data without schemaVersion (fallback)', async () => {
      const legacyData = {
        metadata: {
          setListName: 'Legacy Fallback Show',
          venue: 'Legacy Venue',
          date: '2023-01-01',
          actName: 'Legacy Band'
        },
    ...
    it('should handle data that already has schemaVersion (fallback)', async () => {
      const currentData = {
        schemaVersion: 1,
        metadata: {
          setListName: 'Current Fallback Show',
          venue: 'Current Venue',
          date: '2024-01-01',
          actName: 'Current Band'
        },
